...
class Network:
    ...
    def __init__(self, system=Mimsy.cartesian, reference=(0, 0, 0), station_dims=(.1, .0635, .1), logging=False):
        ...
        self.vector = arrow(axis=(30,0,0), color=(0,0,1), shaftwidth=.01)
        self.object = box(axis=(1,0,0), length=.4, width=.05, height=.2, material=materials.bricks)
        self.prev_R = [0, 0, 0]
    ...
    def update(self, position, R=None):
        self.mimsy.update(position)
        self.vector.axis = (self.mimsy.x() - self.reference[0],
                                    self.mimsy.y() - self.reference[1],
                                    self.mimsy.z() - self.reference[2])
        if R:
            R = [radians(r) for r in R]
            self.object.pos = (self.mimsy.x() - self.reference[0],
                                        self.mimsy.y() - self.reference[1],
                                        self.mimsy.z() - self.reference[2])

            self.object.rotate(angle=-self.prev_R[2], axis=vector(0,0,1))
            self.object.rotate(angle=self.prev_R[1], axis=vector(0,1,0))
            self.object.rotate(angle=self.prev_R[0], axis=vector(1,0,0))

            self.object.rotate(angle=-R[0], axis=vector(1,0,0))
            self.object.rotate(angle=-R[1], axis=vector(0,1,0))
            self.object.rotate(angle=R[2], axis=vector(0,0,1))
